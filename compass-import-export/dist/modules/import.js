...
const get_user_data_file_path_1 = require("../utils/get-user-data-file-path");
...
async function getErrorLogPath(fileName) {
    ...
    const userDataPath = (0, get_user_data_file_path_1.getUserDataFolderPath)();
    ...
    return async (dispatch, getState, { connections, globalAppRegistry: appRegistry, workspaces, track, logger: { log, mongoLogId, debug }, }) => {
        ...
        if (result.aborted) {
            ...
            const onReviewDocumentsClick = appRegistry
                ...
                : undefined;
            if (result.biggestDocSize > 10000000) {
...
const loadTypes = (fields, values) => {
    return async (dispatch, getState, { logger: { log, mongoLogId } }) => {
        ...
        try {
            ...
            log.error(mongoLogId(1001000180), 'Import', 'Failed to analyze CSV fields', err);
            dispatch({
...
const selectImportFileName = (fileName) => {
    return async (dispatch, _getState, { logger: { log, mongoLogId } }) => {
        try {
            ...
            log.info(mongoLogId(1001000189), 'Import', 'Import select file error', {
                fileName,
                error: err?.message,
            });
